        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Izberi sliko</h4>
                </div>
                <div class="modal-body">
                    <div class="image_list"></div>
                </div>
                <div class="modal-footer text-center">
                    <button data-dismiss="modal" class="btn btn-danger btn-lg" type="button">Zapri</button>
                </div>
            </div>
        </div>
    </div>
...
function mk_hurl_upload($name,$caption="",$value="",$id=null,$options=''){
    ...
    <div class="form-group row">
        ...
        <div class="col-lg-3">
            <a id="<?=$id!=null?$id:$name?>_btn" href="#<?=$id!=null?$id:$name?>_uploader" class="btn btn-primary" data-toggle="modal">Naloži</a>
            <!--<a id="choose_<?=$id!=null?$id:$name?>_btn" href="#choose_uploaded_image" onclick="$($(this).attr('href')).attr('insert-to','#<?=$id!=null?$id:$name?>');" class="btn btn-primary" data-toggle="modal">Izberi</a>-->
        </div>
    </div>
<?php
}
function mk_hpostform($action="",$name=null){
    //old cmxform form-horizontal tasi-form"
    ?>
    <form class="form-horizontal"  <?=$name!=null?'name="'.$name.'" id="'.$name.'"':''?> method="post" action="<?=$action?>">
<?php
}
...
function mk_htext($name,$caption="",$value="",$options=''){
    ...
    <div class="form-group row ">
        <label for="<?=$name?>" class="control-label col-lg-2"><?=$caption?></label>
        <div class="col-lg-10">
            <input autocomplete="off" class=" form-control" id="<?=$name?>" name="<?=$name?>" type="text" value="<?=$value?>" <?=$options?>/>
        </div>
    </div>
<?php
}
    ...
    <div class="form-group row">
        ...
        <div class="col-lg-10">
            <textarea class="textarea" placeholder="Place some text here"
                      style="width: 100%; height: 290px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" id="<?=$name?>" name="<?=$name?>" <?=$options?>><?=$value?></textarea>
        </div>
    </div>
<?php
}

function mk_htexteditorpreview($name,$caption="",$value="",$options=''){
    ?>
    <div class="form-group row">
        <label for="<?=$name?>" class="control-label col-lg-2"><?=$caption?></label>
        <div class="col-lg-10">
            <textarea class="textarea disabled" id="<?=$name?>" name="<?=$name?>" <?=$options?>><?=$value?></textarea>
        </div>
    </div>
<?php
}
...
function mk_hsubmit($submitCaption=null,$backURL=null,$backCaption=null){
    ...
    <div class="form-group row">
        <div class="col-lg-offset-2 col-lg-10">
            <input class="btn btn-outline-secondary btn-flat" type="submit" value="<?=$submitCaption!=null?$submitCaption:''?>" >
            <a href="<?=$backURL!=null?$backURL:''?>" class="btn btn-default btn-flat" type="button"><?=$backCaption!=null?$backCaption:''?></a>
        </div>
    </div>
<?php
}
    ...
    <div class="form-group row clearfix">
        ...
        <div class="col-lg-10 col-sm-10">
            <div class="icheck-primary d-inline">
                <input value="<?=$value?>" type="checkbox" class="custom-checkbox-event"  <?=($checked!=null && $checked ==1)?"checked":""?> id="<?=$name?>">
                <label for="<?=$name?>">
                </label>
            </div>
            <input type="hidden" class="custom-checkbox-input" name="<?=$name?>" value="<?=($value!=null&& $value ==1)?1:0?>">
        </div>
    </div>
<?php
}

function mk_hcheckbox_onOff($name,$caption="",$checked=null,$value="1",$options='', $labelOn ="On", $labelOff ="Off"){
    ?>

    <div class="form-group row clearfix">
        <label for="<?=$name?>" class="control-label col-lg-2"><?=$caption?></label>
        <div class="col-lg-10 col-sm-10">
            <input type="checkbox" class="custom-checkbox-change-event" <?=($checked!=null && $checked ==1)?"checked":""?> id="<?=$name?>" data-toggle="toggle" data-onstyle="secondary" data-on="<?=$labelOn?>" data-off="<?=$labelOff?>">
            <input type="hidden" class="custom-checkbox-input" name="<?=$name?>" value="<?=($value!=null&& $value ==1)?1:0?>">
        </div>
    </div>
<?php
...
function mk_hradio_button($name,$caption="",$options_data=null,$field_id,$field_name,$value=null,$that){
    ?>
    <div class="form-group row ">
        <label for="<?=$name?>" class="control-label col-lg-2"><?=$caption?></label>
        <div class="col-lg-10 col-sm-10">
                ...
                <?php foreach($options_data as $data){ ?>
                    <div class="form-check-inline">
                        <label class="form-check-label">
                            <input type="radio" class="form-check-input" name="<?=$name?>" <?=($value!=null && $data[$field_id]==$value)?"checked":""?> value="<?=$data[$field_id]?>"><span style="padding-left: 10px;"><?=_l($data[$field_name],$that)?></span>
                        </label>
                    </div>
                <?php } ?>
            <?php } ?>
