...
class Gbdev_general_admin_model extends CI_Model
{
    ...
    function get_all_cl_extensions_image_position()
    {
        $this->db->select("*");
        $this->db->from('cl_extensions_image_position');
    ...
    function get_all_user()
    {
        $this->db->select("*");
        $this->db->from('users');
        $this->db->where('email !=' , 'admin@gb-dev.si');
...
/*
  ...
  */
    ...
    function get_extensions_rsd()
    {
        $this->db->select('*');
        $this->db->from('extensions');
        $this->db->where("is_rsd",1);
    ...
    function edit_user_tasks($user_task_id,$data){
        $this->db->trans_start();
        $this->db->where('user_tasks_id ',$user_task_id);
        $this->db->where('user_id ',$this->session->userdata['user_id']);
        $this->db->update('user_tasks', $data);
        $this->db->trans_complete();
        // end TRANSACTION
        if ($this->db->trans_status() == FALSE)
        {
    ...
    function insert_user_tasks($data){
        $data['created_date']= time();
        $data['user_id']=$this->session->userdata['user_id'];
    ...
    function get_all_setting_gdpr_options($gdpr_id){
        $this->db->select("*");
        $this->db->from('setting_gdpr_per_lang');
        $this->db->where('setting_gdpr_id',$gdpr_id);
        $query = $this->db->get();
        return $query->result_array();
    }

    function get_all_setting_gdpr(){
        $this->db->select("*");
        $this->db->from('setting_gdpr');
        $query = $this->db->get();
        return $query->result_array();
    }

    function get_all_setting_gdpr_view(){
        $this->db->select("*");
        $this->db->from('setting_gdpr_view');
        $query = $this->db->get();
        return $query->result_array();
    }

    function get_all_setting_legalterm_view(){
        $this->db->select("*");
        $this->db->from('setting_legalterm_view');
        $query = $this->db->get();
        return $query->result_array();
    }
    function insert_setting_legalterm(){
        $data['created_date']= time();
        $data['user_id']=$this->session->userdata['user_id'];
        $this->db->insert('setting_legalterm',$data);
        return $this->db->insert_id();
    }
    function get_all_setting_legalterm_options($legalterm_id){
        $this->db->select("*");
        $this->db->from('setting_legalterm_per_lang');
        $this->db->where('setting_legalterm_id',$legalterm_id);
        $query = $this->db->get();
        return $query->result_array();
    }

    function get_all_setting_legalterm(){
        $this->db->select("*");
        $this->db->from('setting_legalterm');
        $query = $this->db->get();
        return $query->result_array();
    }

    function get_setting_legalterm_detail($setting_legalterm_id)
    {
        $this->db->select("*");
        $this->db->from('setting_legalterm');
        $this->db->where('setting_legalterm_id',$setting_legalterm_id);
        $query = $this->db->get();
        $return = $query->result_array();
        return count($return)!=0?$return[0]:0;
    }
    ...
    function check_setting_legalterm_options($language_id,$setting_legalterm_id){
        $this->db->select("count(*)");
        $this->db->from('setting_legalterm_per_lang');
    ...
    function update_setting_legalterm_activate($setting_legalterm_id){
        $data['valid_from']= date("Y-m-d");
        $data['active']= 1;
        $this->db->where('setting_legalterm_id',$setting_legalterm_id);
        $this->db->update('setting_legalterm', $data);
    }
    function update_setting_legalterm_deactivate($setting_legalterm_id){
        $data['valid_to']= date("Y-m-d");
        $data['active']= 0;
    ...
    function edit_setting_legalterm_options($language_id,$data,$setting_legalterm_id){
        $this->db->where('language_id',$language_id);
        $this->db->where('setting_legalterm_id',$setting_legalterm_id);
        $this->db->update('setting_legalterm_per_lang', $data);
    }
    function insert_setting_legalterm_options($language_id,$data,$setting_legalterm_id){
        $data['language_id']= $language_id;
        $data['created_date']= time();
        $data['setting_legalterm_id']= $setting_legalterm_id;
        $this->db->insert('setting_legalterm_per_lang', $data);
    }
    ...
    function insert_setting_gdpr(){
        $data['created_date']= time();
        $data['user_id']=$this->session->userdata['user_id'];
        $this->db->insert('setting_gdpr',$data);
        return $this->db->insert_id();
    }

    function get_setting_gdpr_detail($setting_gdpr_id)
    {
        $this->db->select("*");
        $this->db->from('setting_gdpr');
        $this->db->where('setting_gdpr_id',$setting_gdpr_id);
        $query = $this->db->get();
        $return = $query->result_array();
        return count($return)!=0?$return[0]:0;
    }
    ...
    function check_setting_gdpr_options($language_id,$setting_gdpr_id){
        $this->db->select("count(*)");
        $this->db->from('setting_gdpr_per_lang');
        $this->db->where('language_id',$language_id);
        $this->db->where('setting_gdpr_id',$setting_gdpr_id);
    ...
    function update_setting_gdpr_activate($setting_gdpr_id){
        $data['valid_from']= date("Y-m-d");
        $data['active']= 1;
        $this->db->where('setting_gdpr_id',$setting_gdpr_id);
        $this->db->update('setting_gdpr', $data);
    }
    function update_setting_gdpr_deactivate($data){
        $data['valid_to']= date("Y-m-d");
        $data['active']= 0;
        $this->db->where('active',1);
        $this->db->update('setting_gdpr', $data);
    ...
    function edit_setting_gdpr_options($language_id,$data,$setting_gdpr_id){
        $this->db->where('language_id',$language_id);
        $this->db->where('setting_gdpr_id',$setting_gdpr_id);
        $this->db->update('setting_gdpr_per_lang', $data);
    }
    function insert_setting_gdpr_options($language_id,$data,$setting_gdpr_id){
        $data['language_id']= $language_id;
        $data['created_date']= time();
        $data['setting_gdpr_id']= $setting_gdpr_id;
        $this->db->insert('setting_gdpr_per_lang', $data);
    }
