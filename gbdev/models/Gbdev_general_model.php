...
class GbDev_general_model extends CI_Model {
    ...
    function get_basic_app_settings(){
        $this->db->select('enabled_reCAPTCHA, reCAPTCHA_secret_key, reCAPTCHA_site_key' );
        $this->db->from('setting');
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    function getNumberOfBlogComments()
    {
        $this->db->select('*');
        $this->db->from('blog_number_of_comments');

        $query = $this->db->get();
        return $query->result_array();
    }

    function getNumberOfBlogCommentsById($id)
    {
        $this->db->select('*');
        $this->db->from('blog_number_of_comments');
    ...
    function getNextBlog($relation_id, $current_blog_created_date)
    {
        $this->db->select('*');
        $this->db->from('extensions');
        $this->db->where('data_type',"page");
        ...
        $this->db->where('status',1);
        $this->db->where('public',1);
        $this->db->where('created_date >',$current_blog_created_date);
        $this->db->order_by("created_date","ASC");
        $this->db->limit(1);
        $query = $this->db->get();
        return $query->result_array();
        //        SELECT * FROM `extensions` WHERE `relation_id`= 12 AND `created_date` > 1523908097 ORDER BY `created_date`ASC LIMIT 1

    }

    function getPreviousBlog($relation_id, $current_blog_created_date)
    {
        $this->db->select('*');
        $this->db->from('extensions');
        $this->db->where('data_type',"page");
        ...
        $query = $this->db->get();
        return $query->result_array();
//SELECT * FROM `extensions` WHERE `relation_id`= 12 AND `created_date` < 1523908097 ORDER BY `created_date`DESC LIMIT 1
    }
/*
...
$this->db->where('tbl_tabel_name != ', $id_name);
*/
    // Select a list from "extensions" table
    function getExtensionsTopProduct($order_by="created_date",$sort="DESC",$limit=null,$offset=0)
    {
        $this->db->select('*');
        $this->db->from('extensions');
        $this->db->where("is_top_product",1);
        $this->db->where('data_type',"page");
        $this->db->where('extensions.language_id',$_SESSION["language"]["language_id"]);
        $this->db->where('status',1);
        $this->db->where('public',1);
    ...
    {
        $this->db->select('setting_gdpr_per_lang.message');
        $this->db->from('setting_gdpr');
        $this->db->join("setting_gdpr_per_lang","setting_gdpr_per_lang.setting_gdpr_id = setting_gdpr.setting_gdpr_id");
        $this->db->where("setting_gdpr.active",1);
        $this->db->where('setting_gdpr_per_lang.language_id',$_SESSION["language"]["language_id"]);
        $query = $this->db->get();
        return $query->row_array();
    }
    ...
    {
        $this->db->select('*,extensions.image, extensions.created_date as ex_created_date');
        $this->db->from('extensions');
...
//        $this->db->where("MATCH(extensions.description)AGAINST('$text')");
    ...
    function getUserByEmailHash($email_hash)
    {
        $this->db->select('*');
        $this->db->from('users');
        $this->db->where('email_hash',$email_hash);
    ...
    function getUserAcceptedLegalTerm($user_id)
    {
        $this->db->select('*');
        $this->db->from('user_active_legal_term_view');
        $this->db->where('user_id',$user_id);
        $query = $this->db->get();
        return $query->row_array();
    }
    function getActiveLegalTerm()
    {
        $this->db->select('*');
    ...
    function getActiveLegalTermByUserLanguageId($language_id)
    {
        $this->db->select('setting_legalterm_per_lang.message');
        $this->db->from('setting_legalterm');
        $this->db->join("setting_legalterm_per_lang","setting_legalterm_per_lang.setting_legalterm_id = setting_legalterm.setting_legalterm_id");
        $this->db->where('setting_legalterm.active',1);
        $this->db->where('setting_legalterm_per_lang.language_id',$language_id);
        $query = $this->db->get();
        return $query->row_array();
    }
    function getUserLanguageById($user_id)
    {
        $this->db->select('*');
        $this->db->from('users');
        $this->db->where('user_id',$user_id);
    ...
    function insertUsersGdpr($data)
    {
        $this->db->insert('users_gdpr',$data);
    }

    function insertUsersLegalTerm($data)
    {
        $this->db->insert('users_legalterm',$data);
    }
     ...
     * This methods used in gbdev_general_helper.php
     */
